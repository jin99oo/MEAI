/*:
 * @plugindesc v.1.5.4 [MZ] Localization of game.
 * @author DKPlugins
 ...
 * @help
 ...
 Download plugin: https://dk-plugins.ru/mz/dktools/
 ...
 3. Primary - Is the language the primary one
 ...
 Starting from version 1.5.0, the plugin has the ability to make language settings for audio
 to a JSON file in the "data/" folder, similar to the language settings.
 You don't need to create the file yourself. When running the game test plugin
 will automatically transfer all the necessary settings to the file "data/AudioLanguages.json".
 After that, you will have to manually add new languages or give instructions to translators.
 The file has the following structure:
 1. Language - Language name
 2. Locale - Locale name
 3. Primary - Is the language the primary one
 ...
 ### Adding a new translation with an example of an event with a message (JSON only) ###
 ...
 ### Adding a new translation (CSV only) ###
 1. Open the csv file.
 ...
 Example for English locale (en) (JSON only):
 ...
 2. DKTools.Localization.audioLocale - Get current audio locale
 3. DKTools.Localization.locales - Get an array of locales
 4. DKTools.Localization.audioLocales - Get an array of audio locales
 5. DKTools.Localization.language - Get current language
 6. DKTools.Localization.audioLanguage - Get current audio language
 7. DKTools.Localization.languages - Get an array of languages
 8. DKTools.Localization.audioLanguages - Get an array of audio languages
 9. DKTools.Localization.getText(text) - Get localized text
 10. DKTools.Localization.getPrevLocale() - Get previous locale
 11. DKTools.Localization.getNextLocale() - Get next locale
 12. DKTools.Localization.getPrevLanguage() - Get previous language
 13. DKTools.Localization.getNextLanguage() - Get next language
 14. DKTools.Localization.selectLocale(locale) - Set locale (async, returns Promise)
 15. DKTools.Localization.selectPrevLocale() - Select previous locale (async, returns Promise)
 16. DKTools.Localization.selectNextLocale() - Select next locale (async, returns Promise)
 17. DKTools.Localization.addChangeLocaleListener(function) - Add "change locale listener" (sync/async function)
 18. DKTools.Localization.getPrimaryLocale() - Get primary locale
 19. DKTools.Localization.getPrimaryLanguage() - Get primary language

 ### 7 ### How to add the options to VisuMZ_1_OptionsCore.js ###
 Attention! Place VisuMZ_1_OptionsCore plugin ABOVE in the plugin list

 Adding a command to change the language:
 1. Enable "Show Options Command" parameter in the Localization plugin
 2. Configure the new option in VisuMZ_1_OptionsCore.js with following parameters:
 Symbol: locale
 JS: Text:
 ...
 Adding a command to change the audio language:
 1. Enable "Show Audio Options Command" parameter in the Localization plugin
 2. Configure the new option in VisuMZ_1_OptionsCore.js with following parameters:
 Symbol: audioLocale
 JS: Text:
 return LocalizationParam.get('Audio Options Command Name');
 JS: Show/Hide:
 return Imported['DKTools_Localization'] && LocalizationParam.get('Show Audio Options Command');
 JS: Draw Option:
 const index = arguments[1];
 const title = this.commandName(index);
 const rect = this.itemLineRect(index);
 ...
 * @command WrapDialogText
 * @desc Wrap all dialog text in tags (show text and choices). See help for details.
 *
 * @arg map
 * @text Map
 * @desc Map. You can specify the ID of a specific map.
 * @type combo
 * @option All maps
 * @option Current map
 * @default All maps
 ...
 * @param Use Audio Languages
 * @text Use audio languages
 * @parent Languages Source
 * @desc Use separate languages for audio files? For example, if you use a dialogue dubbing plugin.
 * @type boolean
 * @default false

 * @param Audio Languages
 * @text Audio languages
 * @parent Use Audio Languages
 * @desc List of audio languages. Only for "Languages source" = "Plugin parameters".
 * @type struct<Language>[]
 * @default ["{\"Language\":\"English\",\"Locale\":\"en\",\"Primary\":\"true\"}"]
 ...
 * @parent Options
 * @desc Show the command in the options for changing the language ?
 * @type boolean
 * @default false

 * @param Options Command Name
 * @parent Show Options Command
 * @desc Command name for changing the language
 * @default {interface_language}

 * @param Show Audio Options Command
 * @parent Options
 * @desc Show the command in the options for changing the audio language ?
 * @type boolean
 * @default false

 * @param Audio Options Command Name
 * @parent Show Audio Options Command
 * @desc Command name for changing the audio language
...
/*:ru
 * @plugindesc v.1.5.4 [MZ] Локализация игры.
 * @author DKPlugins
 * @url https://dk-plugins.ru
 * @target MZ
 * @base DKTools
 ...
 * @help
 ...
 Скачать плагин: https://dk-plugins.ru/mz/dktools/
 ...
 3. Primary - Является ли язык основным
 ...
 Начиная с версии 1.5.0 в плагине появилась возможность вынести настройки языков для аудио
 в JSON файл в папке "data/", аналогично с настройками языков.
 Вам не нужно создавать файл самостоятельно. При запуске теста игры плагин
 автоматически перенесет все необходимые настройки в файл "data/AudioLanguages.json".
 После этого вы должны будете вручную добавлять новые языки, либо дать инструкцию переводчикам.
 Файл имеет следующую структуру:
 1. Language - Название языка
 2. Locale - Название локали
...
Пример JSON файла:
 ...
 ### Добавление нового перевода на примере события с сообщением (для JSON) ###
 ...
 ###===========================================================================
 ## Использование массивов (только для JSON)
 ###===========================================================================
 ...
 Пример для английской локали (en) (только для JSON):
 ...
 2. DKTools.Localization.audioLocale - Получить текущую локаль аудио
 3. DKTools.Localization.locales - Получить массив локалей
 4. DKTools.Localization.audioLocales - Получить массив локалей аудио
 5. DKTools.Localization.language - Получить текущий язык
 6. DKTools.Localization.audioLanguage - Получить текущий язык аудио
 7. DKTools.Localization.languages - Получить массив языков
 8. DKTools.Localization.audioLanguages - Получить массив языков аудио
 9. DKTools.Localization.getText(text) - Получить локализованный текст
 10. DKTools.Localization.getPrevLocale() - Получить предыдущую локаль
 11. DKTools.Localization.getNextLocale() - Получить следующую локаль
 12. DKTools.Localization.getPrevLanguage() - Получить предыдущий язык
 13. DKTools.Localization.getNextLanguage() - Получить следующий язык
 14. DKTools.Localization.selectLocale(locale) - Установить локаль (асинхронная операция, возвращает Promise)
 15. DKTools.Localization.selectPrevLocale() - Выбрать предыдущую локаль (асинхронная операция, возвращает Promise)
 16. DKTools.Localization.selectNextLocale() - Выбрать следующую локаль (асинхронная операция, возвращает Promise)
 17. DKTools.Localization.addChangeLocaleListener(function) - Добавить слушателя изменения локали (синхронная/асинхронная функция)
 18. DKTools.Localization.getPrimaryLocale() - Получить основную локаль
 19. DKTools.Localization.getPrimaryLanguage() - Получить основной язык

 ### 7 ### Как добавить опции в VisuMZ_1_Options_Core.js ###
 Внимание! Поместите плагин VisuMZ_1_Options_Core ВЫШЕ в списке плагинов

 Добавление команды для изменения языка:
 1. Включите параметр "Показать команду изменения языка" в плагине локализации
 2. Добавьте новую опцию в VisuMZ_1_Options_Core.js со следующими параметрами:
 Symbol: locale
 JS: Text:
 return LocalizationParam.get('Options Command Name');
 JS: Show/Hide:
 ...
 Добавление команды для изменения языка аудио:
 1. Включите параметр "Показать команду изменения языка аудио" в плагине локализации
 2. Добавьте новую опцию в VisuMZ_1_Options_Core.js со следующими параметрами:
 Symbol: audioLocale
 JS: Text:
 return LocalizationParam.get('Audio Options Command Name');
 ...
 * @command WrapDialogText
 * @desc Обернуть все тексты диалогов в теги (Показать текст и выбор). Подробности в справке.
 *
 * @arg map
 * @text Карта
 * @desc Карта. Вы можете указать ID конкретной карты.
 * @type combo
 * @option All maps
 * @option Current map
 * @default All maps
 ...
 * @param Use Audio Languages
 * @text Использовать языки игры для аудио
 * @parent Languages Source
 * @desc Использовать отдельные языки для аудио файлов ? Например, если используете плагин озвучивания диалогов.
 * @type boolean
 * @default false

 * @param Audio Languages
 * @text Языки игры для аудио
 * @parent Use Audio Languages
 * @desc Список языков аудио файлов. Только для "Источник языков" = "Параметры плагина".
 * @type struct<Language>[]
 * @default ["{\"Language\":\"Русский\",\"Locale\":\"ru\",\"Primary\":\"true\"}"]
 ...
 * @param Show Options Command
 * @text Показать команду изменения языка
 * @parent Options
 * @desc Показать команду в опциях для изменения языка ?
 * @type boolean
 * @default false

 * @param Options Command Name
 * @text Название команды изменения языка
 * @parent Options
 * @desc Название команды изменения языка
 * @default {interface_language}

 * @param Show Audio Options Command
 * @text Показать команду изменения языка аудио
 * @parent Options
 * @desc Показать команду в опциях для изменения языка аудио ?
 * @type boolean
 * @default false

 * @param Audio Options Command Name
 * @text Название команды изменения языка аудио
 * @parent Show Audio Options Command
 * @desc Название команды изменения языка аудио
 * @default {audio_language}
 ...
 * @param Number Font
 * @desc Шрифт цифр (с расширением файла). Например: my_amazing_font.woff
 * @type combo
 * @option mplus-2p-bold-sub.woff
 * @default mplus-2p-bold-sub.woff
...
Imported['DKTools_Localization'] = '1.5.4';
    ...
    if (extension === '.json') {
        const promises = DKTools.Localization.locales.map(async (locale) => {
            ...
            if (localizationData) {
                ...
                const newFile = new DKTools.IO.File(DKTools.Localization._dataPath + `new_exported_tags_${locale}.json`);
...
PluginManager.registerCommand('DKTools_Localization',
    ...
    const ignoreTags = args.ignoreTags === 'true';
    const regex = DKTools.Localization.regexTag;
    const data = {};
...
DKTools.Localization = class {
     ...
     * @type {String}
     */
    static get _audioLanguagesJsonPath() {
        return 'data/AudioLanguages.json';
    }
    ...
    static async _initializeLanguages() {
        ...
        const useAudioLanguages = LocalizationParam.get('Use Audio Languages');
        let audioLanguages = [];
        let languages;
        ...
        if (source === 'json') {
            ...
            languages = await this._getLanguagesFromJson();

            if (useAudioLanguages) {
                audioLanguages = await this._getAudioLanguagesFromJson();
            }
        } else {
            languages = LocalizationParam.get('Languages');
            audioLanguages = useAudioLanguages ?
                LocalizationParam.get('Audio Languages') : [];
        }
        ...
        audioLanguages.forEach((data) => {
            this._audioLanguages[data['Locale']] = data['Language'];

            if (!this._primaryAudioLanguage && data['Primary']) {
                this._primaryAudioLanguage = data;
            }
        });
    }
    ...
    static async _initializeLocale() {
        ...
        if (locales.length > 0) {
            ...
            if (LocalizationParam.get('Use Audio Languages') && !this._audioLocale) {
                const audioLocales = this.audioLocales;

                if (audioLocales.length > 0) {
                    if (this._primaryAudioLanguage) {
                        this._audioLocale = this._primaryAudioLanguage['Locale'];
                    } else {
                        this._audioLocale = audioLocales[0];

                        if (!firstLaunch) {
                            console.warn('You have not installed the primary audio language! Automatically selected locale: ' + this._audioLocale);
                        }
                    }
                } else {
                    throw new Error(
                        'Add at least one audio language or disable setting "Use Audio Languages"! See help of plugin "DKTools_Localization"!');
                }
            }
     ...
     * @async
     */
    static async _initializeLanguagesJson() {
        const languagesFile = new DKTools.IO.File(this._languagesJsonPath);
        const fonts = LocalizationParam.get('Fonts');
        const helpText = LocalizationParam.get('Help Text');
        let languages = LocalizationParam.get('Languages');

        if (languages.length === 0) {
            languages = [{ Locale: 'en', Language: 'English', Primary: true }];
        }
        ...
        const result = await languagesFile.saveJsonAsync(languages);

        if (result === DKTools.IO.OK) {
            console.log(`Created a file for languages: ${languagesFile.getFullPath()}`);
        } else {
     ...
     * @async
     */
    static async _initializeAudioLanguagesJson() {
        const audioLanguagesFile = new DKTools.IO.File(this._audioLanguagesJsonPath);
        let audioLanguages = LocalizationParam.get('Audio Languages');

        if (audioLanguages.length === 0) {
            audioLanguages = [{ Locale: 'en', Language: 'English', Primary: true }];
        }

        const result = await audioLanguagesFile.saveJsonAsync(audioLanguages);

        if (result === DKTools.IO.OK) {
            console.log(`Created a file for audio languages: ${audioLanguagesFile.getFullPath()}`);
        } else {
            console.error('Localization can not create a file for audio languages');
        }
    }
    ...
    static async _checkCsvData() {
        ...
        const delimiter = LocalizationParam.get('CSV delimiter') || ';';
        const status = await file.saveAsync(`tag${delimiter}${locales.join(delimiter)}\nUSE UTF-8 encoding!!!`);
    ...
    /**
     * Returns true if audio locale is valid
     * @since 1.5.0
     * @static
     * @param {String} locale - Locale
     * @return {Boolean} Locale is valid
     */
    static checkAudioLocale(locale) {
        return this.audioLocales.includes(locale);
    }
     ...
     */
    static async _getLanguagesFromJson(retry = true) {
        const languagesFile = new DKTools.IO.File(this._languagesJsonPath);

        if (!languagesFile.exists() && Utils.isNwjs() && retry) {
            await this._initializeLanguagesJson();

            return await this._getLanguagesFromJson(false);
        } else {
            const result = await languagesFile.loadJsonAsync();
        ...
        DKTools.Utils.throwError(new Error(
            `Can not load languages data from file: ${languagesFile.getFullPath()}`));
    }
     ...
     * @return {Promise<Array>}
     */
    static async _getAudioLanguagesFromJson(retry = true) {
        const audioLanguagesFile = new DKTools.IO.File(this._audioLanguagesJsonPath);

        if (!audioLanguagesFile.exists() && Utils.isNwjs() && retry) {
            await this._initializeAudioLanguagesJson();

            return await this._getAudioLanguagesFromJson(false);
        } else {
            const result = await audioLanguagesFile.loadJsonAsync();
        ...
        DKTools.Utils.throwError(new Error(
            `Can not load audio languages data from file: ${audioLanguagesFile.getFullPath()}`));
    }
    ...
    /**
     * Returns the previous audio locale from the list
     * @since 1.5.0
     * @static
     * @return {String | null} Previous audio locale from the list
     */
    static getPrevAudioLocale() {
        const locales = this.audioLocales;
        let index = locales.indexOf(this._audioLocale);
    ...
    /**
     * Returns the next audio locale from the list
     * @since 1.5.0
     * @static
     * @return {String | null} Next audio locale from the list or null
     */
    static getNextAudioLocale() {
        const locales = this.audioLocales;
        let index = locales.indexOf(this._audioLocale);
    ...
    static getPrimaryLocale() {
        return (this._primaryLanguage && this._primaryLanguage['Locale']) || null;
    }
    ...
    static getPrimaryLanguage() {
        return (this._primaryLanguage && this._primaryLanguage['Language']) || null;
    }
     ...
     */
    static getAudioFolder(folder, filename, locale = this.audioLocale) {
        ...
        const currentLocale = this.audioLocale;
    ...
    static async loadLocale() {
        ...
        if (Utils.isNwjs()) {
            data = await this._file.loadAsync().then(result => result.data);
        } else {
            data = DKTools.IO.WebStorage.load(this._webStoragePath).data;
        }

        const [locale, audioLocale] = (data || '').split(';');

        if (this.checkLocale(locale)) {
            this._locale = locale;

            if (this.checkAudioLocale(audioLocale || locale)) {
                this._audioLocale = audioLocale || locale;
            }
    ...
    static async saveLocale() {
        const data = LocalizationParam.get('Use Audio Languages') ?
            `${this._locale};${this._audioLocale}` : this._locale;

        if (Utils.isNwjs()) {
            await this._file.saveAsync(data, { createDirectory: true });
        } else {
            DKTools.IO.WebStorage.save(this._webStoragePath, data);
        }
    }
    ...
    /**
     * Selects the previous audio locale from the list
     * @since 1.5.0
     * @static
     * @async
     */
    static async selectPrevAudioLocale() {
        return this.selectAudioLocale(this.getPrevAudioLocale());
    }
    ...
    /**
     * Selects the next audio locale from the list
     * @since 1.5.0
     * @static
     * @async
     */
    static async selectNextAudioLocale() {
        return this.selectAudioLocale(this.getNextAudioLocale());
    }
     ...
     * @return {Promise}
     */
    static async selectAudioLocale(locale) {
        if (this.checkAudioLocale(locale)) {
            const previousLocale = this._audioLocale;

            this._audioLocale = locale;

            if (this._audioLocale !== previousLocale || !this.isLocaleFileExists()) {
                await this.saveLocale();
            }
        } else {
            return Promise.reject('You are trying to establish a non-existent audio locale: ' + locale);
        }
    }
...
Object.defineProperties(DKTools.Localization, {
    ...
    /**
     * Locale of the audio files
     * @type {String}
     * @memberof DKTools.Localization
     */
    audioLocale: {
        get: function() {
            return LocalizationParam.get('Use Audio Languages') ?
                this._audioLocale : this.locale;
        },
        configurable: true,
    },
    ...
    audioLanguage: {
        get: function() {
            return LocalizationParam.get('Use Audio Languages') ?
                this._audioLanguages[this._audioLocale] : this.language;
        },
        configurable: true,
    },
    ...
    audioLanguages: {
        get: function() {
            return LocalizationParam.get('Use Audio Languages') ?
                Object.values(this._audioLanguages) : this.languages;
        },
        configurable: true,
    },
    ...
    audioLocales: {
        get: function() {
            return LocalizationParam.get('Use Audio Languages') ?
                Object.keys(this._audioLanguages) : this.locales;
        },
        configurable: true,
    },
...
const Localization_Game_BattlerBase_mostImportantStateText = Game_BattlerBase.prototype.mostImportantStateText;
Game_BattlerBase.prototype.mostImportantStateText = function() {
    return DKTools.Localization.getText(
        Localization_Game_BattlerBase_mostImportantStateText.apply(this, arguments));
};
...
const Localization_Game_Character_startTextPopup = Game_Character.prototype.startTextPopup;
Game_Character.prototype.startTextPopup = function(text, color, duration, posX, posY, secondValue, flag) {
    text = DKTools.Localization.getText(String(text || ''));

    Localization_Game_Character_startTextPopup.call(
        this, text, color, duration, posX, posY, secondValue, flag);
};
...
Game_Actor.prototype.showAddedStates = function() {
    for (const state of this.result().addedStateObjects()) {
        if (state.message1) {
            $gameMessage.add(DKTools.Localization.getText(state.message1).format(this._name));
        }
    }
};
...
Game_Actor.prototype.showRemovedStates = function() {
    for (const state of this.result().removedStateObjects()) {
        if (state.message4) {
            $gameMessage.add(DKTools.Localization.getText(state.message4).format(this._name));
        }
    }
};
...
Window_Options.prototype.makeCommandList = function() {
    ...
    if (!Imported['VisuMZ_1_OptionsCore']) {
        ...
        if (LocalizationParam.get('Use Audio Languages') && LocalizationParam.get('Show Audio Options Command')) {
            this.addAudioLocaleCommand();
        }
    }
};
...
Window_Options.prototype.addAudioLocaleCommand = function() {
    this.addCommand(LocalizationParam.get('Audio Options Command Name'), 'audioLocale');
};
...
Window_Options.prototype.statusText = function(index) {
    ...
    } else if (symbol === 'audioLocale') {
        return DKTools.Localization.audioLanguage;
    }
...
Window_Options.prototype.getConfigValue = function(symbol) {
    if (symbol === 'locale') {
        ...
        return DKTools.Localization.audioLocale;
    }
...
Window_Options.prototype.setConfigValue = function(symbol, volume) {
    if (symbol === 'locale') {
        ...
        DKTools.Localization.selectNextAudioLocale().then(() => {
            if (SceneManager.isCurrentScene(Scene_Options)) {
                SceneManager._scene.onLocaleChange();
            } else {
                this.refresh();
...
Window_BattleLog.prototype.displayItemMessage = function(fmt, subject, item) {
    if (fmt) {
        this.push('addText', DKTools.Localization.getText(fmt).format(subject.name(), item.name));
    }
};
...
Window_BattleLog.prototype.displayAddedStates = function(target) {
    ...
    for (const state of states) {
        const stateText = target.isActor() ?
            DKTools.Localization.getText(state.message1) :
            DKTools.Localization.getText(state.message2);
...
Window_BattleLog.prototype.displayRemovedStates = function(target) {
    ...
    for (const state of states) {
        if (state.message4) {
            this.push('popBaseLine');
            this.push('pushBaseLine');
            this.push('addText', DKTools.Localization.getText(state.message4).format(target.name()));
        }
    }
};
